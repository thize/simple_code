...
class ExpandedTextState extends State<ExpandedText> {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, size) {
      ...
      final double fontSize = result[0] as double;
      final bool textFits = result[1] as bool;
  ...
  List _calculateFontSize(BoxConstraints size, TextStyle style, int maxLines) {
    ...
    final initialFontSize = style.fontSize.clamp(minFontSize, maxFontSize);
  ...
  bool _checkTextFits(
    ...
    if (!widget.wrapWords) {
      final wordCount = text.toPlainText().split(RegExp('\\s+')).length;
      if (maxLines == null) {
        // ignore: parameter_assignments
        maxLines = wordCount;
      } else {
        // ignore: parameter_assignments
        maxLines = maxLines.clamp(1, wordCount).toInt();
      }
    }
